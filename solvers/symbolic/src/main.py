...
from z3 import IntSort, BoolSort, IntVal, BoolVal, is_bv_sort, BitVecVal
import random
import logger
import sexp
import pprint
...
if __name__ == '__main__':
    ...
    if logger.IsLogging(): pprint.pprint(bmExpr)


    numCopies = 1
    while True:
        (spec, specInputPorts, specConn, circuits) = ReadQuery(bmExpr, numCopies)
        if logger.IsLogging():
            print 'Spec:%r'%spec
