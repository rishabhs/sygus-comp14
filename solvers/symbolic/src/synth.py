...
import z3printer
from z3 import Int, And, Bool, Implies, substitute, Not, Sum, Solver, unsat, IntSort 
from z3 import BoolSort, IntVal, BoolVal, BitVecVal, is_bv_sort
import time
import logger
import circuit
        ...
        for j in range(i+1, numCircuits):
            ...
            if circuitX.funcName == circuitY.funcName:
                ...
                if numComponentsX != numComponentsY:
                    raise SynthException('circuits for same function must have same number of components')
                ...
                for comp_no in range(numComponentsX):
                    ...
                    if numInputPortsY != numInputPortsX:
                        raise SynthException('similar components must have same number of input ports')


                    for port_no in range(numInputPortsX):
                        constraints.append(Int(circuitX.PN2LNMap[compX.inputPorts[port_no].name]) ==
                            Int(circuitY.PN2LNMap[compY.inputPorts[port_no].name]))
                    constraints.append(Int(circuitX.PN2LNMap[compX.outputPort.name]) ==
